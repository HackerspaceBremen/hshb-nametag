...
ISR(ADC_vect) {
  ...
  if (adc_values.charging) {
    ...
    } else if (adc_values.ADCvalue <= CHARGE_MAX_ADC) {
      adc_values.percent =
          pgm_read_byte(&chargeLUT[adc_values.ADCvalue - CHARGE_MIN_ADC]);
    } else {
      // percent = pgm_read_byte(&chargeLUT[CHARGE_MAX_ADC-DISCHARGE_MIN_ADC]);
      adc_values.percent = 100;
    }
  } else {
    if (adc_values.ADCvalue < DISCHARGE_MIN_ADC) {
      // percent = pgm_read_byte(&dischargeLUT[0]);
      adc_values.percent = 0;
    } else if (adc_values.ADCvalue <= DISCHARGE_MAX_ADC) {
      adc_values.percent =
          pgm_read_byte(&dischargeLUT[adc_values.ADCvalue - DISCHARGE_MIN_ADC]);
    } else {
      // percent =
