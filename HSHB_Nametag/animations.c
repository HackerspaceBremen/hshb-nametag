...
const uint8_t bArray[] PROGMEM = {5,5,7,10,13,18,23,28,34,39,44,49,53,57,59,61,63,64,63,61,59,57,53,49,44,39,34,28,23,18,13,10,7,5,5};
...
uint8_t circlesAnimation(uint8_t on, uint8_t off) {
	if(counter0<2) {
		...
		counter0=0;
		for(uint16_t k=420; k<522; k++) vRAM[k]=off;
		logoDrawCircle(pgm_read_byte(&circleXPos[i]),pgm_read_byte(&circleYPos[i]),j,on);
