	...
	while (1) {
		...
		if((uint32_t)(millis-lastRefresh)>=20) {
			...
			if(chargeOn) {
				...
				if(slotVals[0]==1) {
					switch(slotVals[1]) {
						case 1: for(uint16_t i = 420; i<522; i++) vRAM[i]=32; break;
						case 2: matrixAnimation(); break;
						case 3: sweepAnimation(32, 1); break;
						case 4: waveAnimation(); break;
						case 5:	linesAnimation(0, animationOnBrightness, animationOffBrightness); break;
						case 6: linesAnimation(1, animationOnBrightness, animationOffBrightness); break;
						case 7: rotateAnimation(0, animationOnBrightness, animationOffBrightness); break;
						case 8: rotateAnimation(1, animationOnBrightness, animationOffBrightness); break;
						case 9: circlesAnimation(animationOnBrightness, animationOffBrightness); break;
						default: for(uint16_t i = 420; i<522; i++) vRAM[i]=0; break;
					}
				
					if(slotVals[3]==1) {
						writeText(slotVals[4], textBuffer, slotVals[5], slotVals[6], textOnBrightness, textOffBrightness);
						if(slotVals[2] && (millis-slotLoadMillis)/1000>=slotVals[2]) newSlot=1;
						...
						if(!scrollingDisplay()) {
							...
							if(slotVals[2] && scrollCounter>slotVals[2]) {
								...
								writeScroll(textBuffer, slotVals[5], slotVals[6], textOnBrightness, textOffBrightness);
							}
						}
					} else {
						for(uint16_t i = 0; i<420; i++) vRAM[i]=0;
				...
				if(newSlot) {
					...
					scrollStop();
					if(slotVals[0] == 1) {
						for(uint8_t i=1; i<7; i++) slotVals[i] = eeprom_read_byte((const uint8_t*)(slot*100+i));
						if(slotVals[3]) {
							uint8_t skipByte=0;
							for(uint8_t i=0; i<90; i++) {
								textBuffer[i] = eeprom_read_byte((const uint8_t*)(slot*100+7+i+skipByte));
								if(textBuffer[i]==195) {
									skipByte++;
									textBuffer[i] = eeprom_read_byte((const uint8_t*)(slot*100+7+i+skipByte));
								}
		...
		if(uart_rx) {
			...
			/*if(rx_buf[0]=='H' || rx_buf[0]=='h') {
				if(rx_buf[1]==' ' && (rx_buf[2]=='W' || rx_buf[2]=='w')) {
					uartWriteLn(F("W <Slot> <Animation> <Duration (in s)> <Text Type> <X Offset> <Scaler> <Char Width> <Text>"));
				} else if(rx_buf[1]==' ' && (rx_buf[2]=='R' || rx_buf[2]=='r')) {
					uartWriteLn(F("R <Slot>"));
				} else if(rx_buf[1]==' ' && (rx_buf[2]=='D' || rx_buf[2]=='d')) {
					...
					uartWriteLn(F("B <Animation On Brightness> <Animation Off Brightness> <Text On Brightness> <Text Off Brightness>"));
				} else {
					uartWriteLn(F("W: Write slot\r\nR: Read slot\r\nD: Disable slot\r\nE: Enable slot\r\nF: Format\r\nL: List active slots\r\nB: Brightness"));
				}
			} else */if(rx_buf[0]=='W' || rx_buf[0]=='w') {
				msg_pos+=2;
				...
				for(uint8_t i=0;i<rx_len-msg_pos;i++) {
					eeprom_write_byte((uint8_t*)(values[0]*100+7+i), rx_buf[msg_pos+i]);
					eeprom_write_byte((uint8_t*)(values[0]*100+7+rx_len-msg_pos), 0);
				}
				
				if(values[3]) {
					tx_len=snprintf(tx_buf, TX_BUFFER_SIZE, "Slot %d:\r\n%d %d %d %d %d %d %d\r\n", values[0], values[0], values[1], values[2], values[3], values[4], values[5], values[6]);
					for(uint8_t i=0; i<90; i++) {
						tx_len++;
						tx_buf[tx_len] = eeprom_read_byte((const uint8_t*)(values[0]*100+7+i));
					...
					skip_string: tx_len=snprintf(tx_buf, TX_BUFFER_SIZE, "Slot %d:\r\n%d %d\r\nNo Text\r\n", values[0], values[1], values[2]);
				}
				
				uart_tx=1;
				
				if(0) {
					skipW: uartWriteLn(F("W <Slot> <Animation> <Seconds/Scrolls> <Text Type> <Offset/Scroll Speed> <Scaler> <Char Space> <Text>"));
				}
				...
				if(rx_len==3 && rx_buf[1]==' ' && rx_buf[2]>='0' && rx_buf[2]<='9') {
					...
					if(eeprom_read_byte((const uint8_t*)(slotNum*100)) != 1) {
						tx_len=snprintf(tx_buf, TX_BUFFER_SIZE, "Slot %d is not active.\r\n", slotNum);
					} else {
						uint8_t values[7] = {0,0,0,0,0,0,0};
						// [0] Slot, [1] Animation, [2] Duration, [3] Text Type
						// [4] X Offset, [5] Scaler, [6] Char Width
						values[0]=slotNum;
						for(uint8_t i=1; i<7; i++) values[i] = eeprom_read_byte((const uint8_t*)(slotNum*100+i));
					
						if(values[3]!=0) {
							tx_len=snprintf(tx_buf, TX_BUFFER_SIZE, "Slot %d:\r\n%d %d %d %d %d %d %d\r\n", slotNum, values[0], values[1], values[2], values[3], values[4], values[5], values[6]);
							for(uint8_t i=0; i<90; i++) {
								tx_len++;
								tx_buf[tx_len] = eeprom_read_byte((const uint8_t*)(slotNum*100+7+i));
				...
				if(rx_len==3 && rx_buf[1]==' ' && rx_buf[2]>='0' && rx_buf[2]<='9') {
					...
					tx_len=snprintf(tx_buf, TX_BUFFER_SIZE, "Slot %d disabled.\r\n", slot);
				} else {
					uartWriteLn(F("D <Slot>"));
				}
				...
				if(rx_len==3 && rx_buf[1]==' ' && rx_buf[2]>='0' && rx_buf[2]<='9') {
					...
					tx_len=snprintf(tx_buf, TX_BUFFER_SIZE, "Slot %d enabled.\r\n", slot);
					uart_tx=1;
				} else {
					uartWriteLn(F("E <Slot>"));
				}
				...
				if(rx_len>=3 && rx_buf[1]==' ') {
					if(rx_buf[2]>='0' && rx_buf[2]<='9') {
						...
						tx_len=snprintf(tx_buf, TX_BUFFER_SIZE, "Slot %d cleared\r\n", slot);
					} else if((rx_buf[2]>='A' || rx_buf[2]<='a') && (rx_buf[3]>='L' || rx_buf[3]<='l') && (rx_buf[4]>='L' || rx_buf[4]<='l')) {
					...
					invalidF: uartWriteLn(F("F <Slot/All>"));
				}
			}  else if(rx_len==1 && (rx_buf[0]=='L' || rx_buf[0]=='l')) {
				tx_len=0;
				for(uint8_t i=0; i<10; i++) {
					if(eeprom_read_byte((const uint8_t*)(i*100)) == 1) {
						tx_len+=snprintf(tx_buf+tx_len, TX_BUFFER_SIZE-tx_len, "%d: ", i);
						uint8_t values[7] = {0,0,0,0,0,0,0};
						// [0] Slot, [1] Animation, [2] Duration, [3] Text Type
						// [4] X Offset, [5] Scaler, [6] Char Width
						values[0]=i;
						for(uint8_t j=1; j<7; j++) values[j] = eeprom_read_byte((const uint8_t*)(i*100+j));
						
						if(values[3]!=0) {
							for(uint8_t j=0; j<8; j++) {
								tx_len++;
								tx_buf[tx_len] = eeprom_read_byte((const uint8_t*)(i*100+7+j));
								if(tx_buf[tx_len]==0) {
									tx_buf[tx_len]='\r';
									tx_buf[tx_len+1]='\n';
									tx_len+=2;
							...
							tx_len+=snprintf(tx_buf+tx_len, TX_BUFFER_SIZE-tx_len, "-\r\n");
						}
					}
				}
				if(tx_len==0) {
				...
				if(0) {
					skipB: uartWriteLn(F("B <Logo Bright> <Logo Dark> <Text Bright> <Text Dark>"));
				}
			} else {
				//uartWriteLn(F("Invalid command. Available commands:\r\nW: Write Slot\r\nR: Read Slot\r\nE: Enable Slot\r\nD: Disable Slot\r\nF: Format\r\nL: List\r\nB: Brightness"));
				uartWriteLn(F("Invalid command.\r\nW: Write\r\nR: Read\r\nE: Enable\r\nD: Disable\r\nL: List\r\nF: Format\r\nB: Brightness"));
			}
...
void checkButton() {
	...
	if(button && newButtonEvent && millis-buttonPressMillis>100 && millis-buttonPressMillis<1000) {
		newButtonEvent=0;
		newSlot=1;
	} else if(!button && newButtonEvent && millis-buttonPressMillis>LONG_PRESS_TIME) {
		newButtonEvent=0;
		...
		shutdownAfterButton: displayOn=0;	// Disable Display
		...
		while(charging) {
			...
			if(!button && newButtonEvent && millis-buttonPressMillis>LONG_PRESS_TIME) {
				newButtonEvent=0;
		...
		skipDeepSleep: snprintf(battBuffer, BATT_BUFFER_SIZE, "Start %03d", BOARD_ID);
		for(uint16_t i = 0; i<522; i++) vRAM[i]=0;
		displayOn=1;	// Enable Display
		for(uint8_t i=0; i<=64; i++) {
	...
	} else if(!button && newButtonEvent && millis-buttonPressMillis>MEDIUM_PRESS_TIME) {
		fillLogoPercent(percent, 19, 32, 1);
		snprintf(battBuffer, BATT_BUFFER_SIZE, "Battery %d%%", percent);
		writeText(0, battBuffer, 1, 0, 64, 0);
		_delay_ms(500);
...
ISR(ADC_vect) {
	...
	if(charging) {
		...
		if(ADCvalue<DISCHARGE_MIN_ADC) {
			...
			//percent = pgm_read_byte(&dischargeLUT[DISCHARGE_MAX_ADC-DISCHARGE_MIN_ADC]);
			percent = 100;
		}
	}
